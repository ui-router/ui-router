...
npm_config_ignore_scripts=true npm exec --yes --package "@angular/cli@$VERSION" -- ng new --package-manager=npm --style=css --routing=false --skip-git --skip-install --skip-tests "$DIR"
rm -rf $DIR/src/app
...
$JSON write $DIR/package.json scripts.test:dev "ng build --configuration development && DIST_PATH=dist/$DIR/browser npm exec -- playwright test"
$JSON write $DIR/package.json scripts.test:prod "ng build  --configuration production && DIST_PATH=dist/$DIR/browser npm exec -- playwright test"
